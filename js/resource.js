...
const rooms = [
    {
        id: 1,
        title: 'OWASP Top 10',
        description: 'Learn about the OWASP Top 10 vulnerabilities and how to exploit them',
        category: 'web',
        difficulty: 'easy',
        type: 'free',
        duration: '4 hours',
    ...
    },
    {
        ...
        description: 'Master the Linux command line and essential system administration',
        category: 'linux',
        difficulty: 'easy',
        type: 'free',
        duration: '3 hours',
...
function renderLearningPaths() {
    learningPathsContainer.innerHTML = learningPaths.map(path => `
        <div class="path-card">
            ${path.progress > 0 ? `
            ...
            <div class="path-card-header">
                <div class="path-icon" style="background-color: ${path.iconColor}20;">
                    <span style="font-size: 24px;">${path.icon}</span>
                </div>
                <div class="path-badge ${path.free ? 'badge-free' : 'badge-premium'}">
                    ${!path.free ? `
...
function renderRooms() {
    ...
    // Update count
    roomsCountElement.textContent = `${filteredRooms.length} room${filteredRooms.length !== 1 ? 's' : ''}`;
    ...
    if (filteredRooms.length === 0) {
        ...
        roomsGridContainer.innerHTML = filteredRooms.map(room => `
            <div class="room-card">
                <div class="room-image">
                    <img src="${room.image}" alt="${room.title}" loading="lazy">
                    <div class="room-badges">
                        <div class="difficulty-badge difficulty-${room.difficulty}">${room.difficulty}</div>
                        ${room.type === 'premium' ? '<div class="type-badge">Premium</div>' : ''}
                    </div>
                ...
                <div class="room-content">
                    ...
                    <div class="room-completion">
                        <div class="room-completion-header">
                            ...
                            <span class="room-completion-value">${room.completionRate}%</span>
                        </div>
                        <div class="room-completion-bar">
                            <div class="room-completion-bar-fill" style="width: ${room.completionRate}%"></div>
                        </div>
                    </div>
